...
async function run(reader) {
    ...
    const app = await (0, _client.client)("https://sonali-behera-bear-breed-classifier.hf.space/");
...
var global = arguments[3];
var Buffer = require("77ebc52fe9fbe75b").Buffer;
...
async function client(app_reference, options = {
    ...
    return new Promise(async (res)=>{
        ...
        if (typeof window === "undefined" || !("WebSocket" in window)) {
            const ws = await require("41d5cecd7f81ddb8");
        ...
        function predict(endpoint, data, event_data) {
            ...
            return new Promise((res2, rej)=>{
                ...
                app.on("data", (d)=>{
                    data_returned = true;
                    if (status_complete) app.destroy();
                    res2(d);
                }).on("status", (status)=>{
                    if (status.stage === "error") rej(status);
                    if (status.stage === "complete" && data_returned) app.destroy();
                    if (status.stage === "complete") status_complete = true;
                });
            });
        }
        ...
        async function view_api(config2) {
            ...
            try {
                let response;
                if (semiver(config2.version || "2.0.0", "3.30") < 0) response = await fetch("https://gradio-space-api-fetcher-v2.hf.space/api", {
                    method: "POST",
                    body: JSON.stringify({
                        serialize: false,
                ...
                else response = await fetch(`${config2.root}/info`, {
                    headers
                });
...
function transform_output(data, api_info, root_url, remote_url) {
    let transformed_data = data.map((d, i)=>{
        var _a, _b, _c, _d;
        if (((_b = (_a = api_info.returns) == null ? void 0 : _a[i]) == null ? void 0 : _b.component) === "File") return normalise_file(d, root_url, remote_url);
        else if (((_d = (_c = api_info.returns) == null ? void 0 : _c[i]) == null ? void 0 : _d.component) === "Gallery") return d.map((img)=>{
            return Array.isArray(img) ? [
                normalise_file(img[0], root_url, remote_url),
                img[1]
            ] : [
                normalise_file(img, root_url, remote_url),
                null
            ];
        });
        else if (typeof d === "object" && d.is_file) return normalise_file(d, root_url, remote_url);
        else return d;
    });
...
function normalise_file(file, root, root_url) {
    ...
    if (typeof file === "string") return {
        ...
        for (const x of file)if (x === null) normalized_file.push(null);
        else normalized_file.push(normalise_file(x, root, root_url));
        return normalized_file;
    } else if (file.is_file) {
        if (!root_url) file.data = root + "/file=" + file.name;
        else file.data = "/proxy=" + root_url + "/file=" + file.name;
    }
    return file;
}
function get_type(type, component, serializer, signature_type) {
    ...
    if (serializer === "JSONSerializable" || serializer === "StringSerializable") return "any";
        ...
        else return signature_type === "parameter" ? "Blob | File | Buffer" : `{ name: string; data: string; size?: number; is_file?: boolean; orig_name?: string}`;
...
async function handle_blob(endpoint, data, api_info, token) {
    ...
    return new Promise((res)=>{
        Promise.all(blob_refs.map(async ({ path , blob , data: data2 , type  })=>{
            if (blob) {
                const file_url = (await upload_files(endpoint, [
                    blob
                ], token)).files[0];
                return {
                    path,
            ...
            r.forEach(({ path , file_url , base64 , type  })=>{
                if (base64) update_object(data, base64, path);
                else if (type === "Gallery") update_object(data, file_url, path);
                else if (file_url) {
                    const o = {
                        is_file: true,
                        name: `${file_url}`,
...
async function walk_and_store_blobs(param, type, path = [], root = false, api_info) {
    if (Array.isArray(param)) {
        ...
        if (type === "Image") {
            let data;
            if (typeof window !== "undefined") data = await image_to_data_uri(param);
            else {
...
},{"77ebc52fe9fbe75b":"fCgem","41d5cecd7f81ddb8":"1UJnQ","83ccb8ffbee1bf02":"i9ltP","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"fCgem":[function(require,module,exports) {
/*!
...
},{}],"1UJnQ":[function(require,module,exports) {
module.exports = require("819a2665cfab2747")(require("6f442d74a7e9db25").getBundleURL("UckoE") + "wrapper-b7460963.3de2e45d.js" + "?" + Date.now()).catch((err)=>{
    delete module.bundle.cache[module.id];
    throw err;
}).then(()=>module.bundle.root("92XUw"));

},{"819a2665cfab2747":"61B45","6f442d74a7e9db25":"lgJ39"}],"61B45":[function(require,module,exports) {
"use strict";
...
},{}],"i9ltP":[function(require,module,exports) {
module.exports = Promise.resolve(module.bundle.root("fCgem"));
...
},{}]},["2UeK4","bB7Pu"], "bB7Pu", "parcelRequire1ab5")
